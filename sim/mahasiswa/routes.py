...
from sim.mahasiswa.forms import mahasiswaF, loginmahasiswaF, editmahasiswaF, pengaduan_F, editpengaduan_F
...
@rmahasiswa.route("/edit_pengaduan", methods=['GET', 'POST'])
@login_required
def edit_pengaduan():
    dt_pengaduan=Tpengaduan.query.filter_by(mahasiswa_id=current_user.id)
    form=pengaduan_F()
    if form.validate_on_submit():
        ...
        return redirect(url_for('rmahasiswa.edit_pengaduan'))
    return render_template('edit_pengaduan.html', form=form, dt_pengaduan=dt_pengaduan)
